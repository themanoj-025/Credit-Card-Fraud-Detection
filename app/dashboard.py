...
st.markdown("*Real-time transaction monitoring with AI-powered fraud detection and SHAP explainability*")
...
flagged_count = sum(1 for t in st.session_state.transactions if t.get('is_fraud', False))
...
with col5:
    st.metric("💸 Review Costs", f"${st.session_state.total_review_cost:,.2f}", delta_color="inverse")
...
if st.session_state.get('run_batch', False):
    ...
    for i in range(st.session_state.get('batch_size', 5)):
        ...
        status.text(f"Processed transaction {i+1}/{st.session_state.get('batch_size', 5)}...")
...
if st.session_state.transactions:
    ...
    for i, txn in enumerate(recent):
        ...
        with st.expander(f"{icon} Transaction #{total_txns - i} — {badge} — ${txn.get('Amount', 0):.2f}", expanded=is_fraud):
            c1, c2, c3 = st.columns(3)
            with c1:
                st.write(f"**Amount:** ${txn.get('Amount', 0):.2f}")
                st.write(f"**Time:** {txn.get('Time', 0):.0f}s")
                st.write(f"**True Class:** {'Fraud' if txn.get('true_class') else 'Legitimate'}")
            with c2:
    ...
    with col_a:
        ...
        fig_prob.add_vline(x=predictor.threshold if predictor else 0.5,
    ...
    with col_b:
        ...
        fig_impact.add_trace(go.Scatter(y=cumulative_saved, name='Fraud Caught ($)',
                                        fill='tozeroy', line=dict(color='#38ef7d')))
        fig_impact.add_trace(go.Scatter(y=cumulative_lost, name='Fraud Missed ($)',
                                        fill='tozeroy', line=dict(color='#ff416c')))
        fig_impact.add_trace(go.Scatter(y=cumulative_cost, name='Review Costs ($)',
                                        fill='tozeroy', line=dict(color='#ffa726')))
        fig_impact.update_layout(title="Cumulative Business Impact",
                                xaxis_title="Transactions", yaxis_title="USD ($)")
    ...
    with bc1:
        st.metric("Fraud Caught", f"{st.session_state.total_caught}",
                  f"${st.session_state.total_saved:,.0f} saved")
    with bc2:
        st.metric("Fraud Missed", f"{st.session_state.total_missed}",
                  f"${st.session_state.total_missed * 150:,.0f} lost")
    with bc3:
        flagged_total = sum(1 for t in st.session_state.transactions if t.get('is_fraud'))
        st.metric("Reviews Conducted", f"{flagged_total}",
