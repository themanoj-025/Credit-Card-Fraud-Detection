...
class FeatureEngineer:
    ...
    def transform(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.create_bins:
            ...
            X['Amount_bin'] = pd.cut(
                X['Amount'], bins=[0, 10, 50, 200, 1000, 30000],
                labels=[0, 1, 2, 3, 4]
            ).astype(float)
        ...
        if self.create_bins:
            ...
            # Time since previous transaction (approximation using sorted time)
        ...
        if self.create_interactions:
            # V14 and V4 are typically the most important for fraud
        ...
        logger.info(f"Feature engineering: {df.shape[1]} → {X.shape[1]} features")
        return X
...
if __name__ == "__main__":
    ...
    fe = FeatureEngineer()
    X_enriched = fe.transform(df)
    
    print(f"\nOriginal features: {len(FeatureEngineer.get_base_features())}")
    print(f"Enriched features: {X_enriched.shape[1]}")
    print(f"\nNew columns: {[c for c in X_enriched.columns if c not in FeatureEngineer.get_base_features()]}")
